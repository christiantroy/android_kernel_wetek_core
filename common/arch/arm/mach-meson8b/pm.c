...
#ifdef CONFIG_WETEK_CORE_LITE
	if(power_on == OFF) {
        printk("Power LED red color");
        amlogic_gpio_request_one(GPIOAO_13,GPIOF_OUT_INIT_HIGH,"power_led");
    } else {
        printk("Power BLUE red color");
        amlogic_gpio_request_one(GPIOAO_13,GPIOF_OUT_INIT_LOW,"power_led");
    }
#else
    if(power_on == OFF) {
